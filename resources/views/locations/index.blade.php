...
@section('header', '📍 จัดการสถานที่')
@section('subtitle', 'เพิ่ม/ลบ/แก้ไข สถานที่สำหรับจัดเก็บอุปกรณ์')

@section('content')
    <div id="locations-page" class="page animate-slide-up-soft">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {{-- ซ้าย: ฟอร์มเพิ่มสถานที่แบบคงที่ --}}
            <div class="lg:col-span-1">
                <div class="soft-card rounded-2xl p-6 gentle-shadow sticky top-24">
                    <h3 class="text-xl font-bold gradient-text-soft mb-4">📍 เพิ่มสถานที่ใหม่</h3>
                    ...
                    {{-- Create form (RESTful: POST -> locations.store) --}}
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    required
                                    value="{{ old('name') }}"
                                    placeholder="เช่น คลังกลาง ชั้น 2, ห้อง Server, อาคาร A"
            ...
            {{-- ขวา: ตารางรายการสถานที่ --}}
            <div class="lg:col-span-2">
                <div class="soft-card rounded-2xl p-6 gentle-shadow">
                    <h3 class="text-xl font-bold text-gray-800 mb-4">รายการสถานที่ทั้งหมด</h3>

                    <div class="overflow-x-auto scrollbar-soft">
                        <table class="w-full">
                            <thead class="bg-gradient-to-r from-blue-50 to-purple-50">
                                <tr>
                                    <th class="px-4 py-3 text-left font-medium text-gray-700 text-sm">ชื่อสถานที่</th>
...
@push('scripts')
<script>
    // ดักจับ event click ของทุกปุ่มที่มี class 'delete-button'
    document.querySelectorAll('.delete-button').forEach(button => {
        button.addEventListener('click', function(event) {
            event.preventDefault(); // หยุดการทำงานปกติของปุ่ม

            // หา form ที่ใกล้ที่สุดของปุ่มที่ถูกกด
            ...
            Swal.fire({
                ...
                customClass: {
                    // (Optional) เพิ่ม class เพื่อให้เข้ากับธีม soft-card ของคุณ
                ...
                if (result.isConfirmed) {
                    // ถ้าผู้ใช้กดยืนยัน ให้ทำการ submit form
                    form.submit();
                }
            })
        });
